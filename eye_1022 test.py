...
# 創建 user_id 表格(關聯式資料庫)
    ...
    def edit_onchange(self):  #編輯使用者介面 : 顯示用戶歷史資料
        ...
        all_buttons = self.temp_button1_edit + self.temp_button2_edit + self.temp_button3_edit + self.temp_button4_edit + self.temp_button5_edit 

        checkboxes = [
            self.use_situation4_combobox_edit,
            self.use_situation5_combobox_edit,
    ...
    def capture_camera_frame(self,save_path="/Users/chiahsin/Eye_Myself/1117/normal"):


        if not os.path.exists(save_path):
            os.makedirs(save_path)  # 創建資料夾，包含所有父資料夾
    
        brightness_value = self.brightness_value  # 當前亮度值
        blink_count = self.blink_counter  # 當前眨眼次數
        ...
        if  self.status == 'start' and self.camera is not None and self.camera.isOpened():
            ...
            # 生成文件名並保存截圖
                ...
                filename = os.path.join(save_path, f"{timestamp}_camera.png")
                cv.imwrite(filename, frame)  # 保存截圖到指定路徑
    ...
    def pushButton_Exhausted_onchange(self):
        ...
        if self.status in ['start', 'rest']:  
            if not self.is_exhausted:  # 第一次按下，進入疲勞狀態
                ...
                self.showMainWindow("Exhausted state started. Auto capturing images.", line=False)


                # 檢查定時器是否已啟動，未啟動則開始
                if not self.timer_camera.isActive():  # 檢查定時器是否已經啟動
                    self.timer_camera.start(self.capture_interval)  # 如果未啟動，則開始定時器
                ...
                exhausted_path ="/Users/chiahsin/Eye_Myself/1117/exhausted"

                if not os.path.exists(exhausted_path):
                     os.makedirs(exhausted_path)
                     
                 #定時自動保存照片到 "exhausted" 資料夾
                try:
                    self.timer_camera.timeout.disconnect()  # 先斷開舊的連接，避免多次連接信號
                except:
                     pass

                 # 定時自動保存照片到 "exhausted" 資料夾
                self.timer_camera.timeout.connect(lambda: self.capture_camera_frame(save_path=exhausted_path))
        ...
        elif self.is_exhausted:  # 第二次按下按鈕，恢復正常狀態
...
def copy_and_rename_database_file(user_name):
    try:
        ...
        target_directory = "/Users/chiahsin/Eye_Myself/1117" # 指定目標目錄
...
def upload_to_google_drive(file_paths):
    try:
        ...
        SERVICE_ACCOUNT_FILE = "/Users/linyiwen/Desktop/畢業專題/eye-myself-357cdddd6407.json"
