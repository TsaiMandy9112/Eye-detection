...
# 創建 user_id 表格(關聯式資料庫)
    ...
    def update_threshold_values(self):   # page1 開始前調整閾值
        ...
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?,?,?)
            """
            self.cursorObj.execute(insert_query, (current_user,self.line_token_input.text(),self.eye_area_record,distance_ratio, brightness, blink, blink_num, insert_time))
    ...
    def save_numth_to_new_db(self):  # page2 開始後更新閾值功能
        ...
        try:
            ...
            self.cursorObj.execute(
                insert_query, 
                (user, self.line_token_input.text(), self.eye_area_record, distance_record, brightness_record, blink_record, blink_per_minute_record, insert_time)
            )
    ...
    def user_list_onchange(self, user=1):
        ...
        print(f"目前選擇的使用者: {self.current_user}")
        ...
        # 查詢 threshold 表中的所有數據並打印出來（調試用）
        print("threshold 表的所有數據：")
        all_data_query = "SELECT * FROM threshold"
        all_data = self.cursorObj.execute(all_data_query).fetchall()
        for row in all_data:
            print(row)

        # 查詢該 user 的最後一筆插入資料（依據 ROWID）
        query = """
        ...
        if result:
            ...
            print(f"查詢到的最後插入數據，提取自資料表 threshold，對應使用者: {self.current_user}")
            print(f"資料內容: {result}")
...
def copy_and_rename_database_file(user_name):
    try:
        ...
        # 原始的資料庫檔案路徑
        original_file_name = os.path.join(current_dir, 'database.db')
            
        # 指定目標目錄為當前程式碼的目錄
        target_directory = current_dir

        # 生成新的檔案名稱，包含使用者名稱
        new_file_name = os.path.join(target_directory, f"{user_name}_database.db")

        # 使用 shutil 複製檔案並重新命名
        shutil.copyfile(original_file_name, new_file_name)
        print(f"Database file copied and renamed to {new_file_name}")

        return [new_file_name]  # 回傳新檔案的路徑
    except Exception as e:
...
def upload_to_google_drive(file_paths):
    try:
        ...
        # 使用服務帳戶的 JSON 憑證檔案路徑
        SERVICE_ACCOUNT_FILE = os.path.join(script_dir, 'eye-myself-357cdddd6407.json')
